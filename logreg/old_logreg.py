...
class LogReg:
    ...
    def sg_update(self, train_example, iteration,
        ...
        pi = sigmoid(np.dot(self.beta, train_example.x))
        ...
        # self.beta = self.finalize_lazy(iteration)
        indices = np.nonzero(train_example.x != 0)
        print ("last update", self.last_update)
        if len(indices)>0:
        	np.put(self.last_update, indices, [iteration * len(indices)])
        
        # self.beta = self.beta * np.power((1 - 2 * self.step(iteration) * self.mu), (iteration - self.last_update)) + self.step(iteration) * (train_example.y - pi) * train_example.x
